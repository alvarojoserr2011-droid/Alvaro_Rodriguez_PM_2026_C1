...
En este caso se copia la cadena cad0 a cad1. Si el espacio reservado para cad1 es
...
printf("\nPrueba de la función strcpy. Se copia la cadena XX a cad1: %s\n", cad1);
    strncpy(cad2, cad0, 4)
    cad2[4] = '\0';
/* La función strncpy permite copiar un número determinado de caracteres a otra cadena de caracteres. En este caso se copian 4 caracteres de la cadena
cad0—segundo argumento— a cad2—primer argumento.
Siempre se debe incorporar al final de la cadena el caracter de terminación.
Si el espacio
reservado para cad2 es menor que lo que se pretende copiar, se genera un error en la ejecución del programa. */
...
En este caso se agrega la cadena cad3 a cad0. Si el espacio
reservado para cad0 es menor a lo que se debe almacenar se genera un error de ejecución. */
...
/* La función strstr se utiliza para localizar una cadena de caracteres dentro de otra cadena. Si la encuentra, regresa un apuntador al inicio de la
